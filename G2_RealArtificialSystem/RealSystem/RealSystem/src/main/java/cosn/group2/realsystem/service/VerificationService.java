...
public class VerificationService {
    ...
    public boolean isUser(String tokenBearer) throws Exception {
        ...
        final Supplier<Boolean> isUserSupplier = () -> {
            ...
            try {
                ...
                return response.getStatusCode().equals(HttpStatus.OK)
                        // and if user is ServiceProvider
                        && "ServiceProvider".equals(response.getBody().role);
            } catch (Exception e) {
                e.printStackTrace();
            }
        ...
        if (!!ENABLE_CIRCUIT_BREAKER) isUserSupplier.get();
        return circuitBreaker.run(isUserSupplier, throwable -> {
            LOG.warn("Error making request to gateway service", throwable);
